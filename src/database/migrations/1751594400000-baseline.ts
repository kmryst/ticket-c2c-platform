// ファイル概要:
// このファイルは TypeORM versioned migrations の baseline（初期スキーマ）です（Issue #92）。
// 2026-07-04 時点の database/schema.sql のスナップショットを凍結したもので、
// events / ticket_inventory / purchases と idempotency 用 index を作成します。
//
// 運用ルール:
// - このファイルは今後編集しない（凍結）。スキーマ変更は新しい migration ファイルを追加する
//   （npm run migration:create -- src/database/migrations/<Name> で雛形を作り、SQL を手書きする）。
// - ローカル PoC の正本 database/schema.sql も同じ PR で同期更新する（CLAUDE.md「DB スキーマ」）。
// - DDL は冪等（IF NOT EXISTS / DO block）だが、適用済みかどうかの判定は
//   typeorm_migrations table の履歴で行われるため、通常このファイルが再実行されることはない。
...
export class Baseline1751594400000 implements MigrationInterface {
  name = 'Baseline1751594400000';
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // baseline の巻き戻しは PoC データごとテーブルを落とす破壊的操作になる。
    // dev / staging はエフェメラル環境（ADR-0008）で destroy が正規の巻き戻し手段のため、
    // 誤実行を防ぐ目的で down は明示的にエラーにする。
    void queryRunner;
    throw new Error(
      'baseline migration is irreversible: recreate the environment instead (ADR-0008)',
    );
  }
}
