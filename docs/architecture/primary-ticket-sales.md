...
```text
held
  -> payment_processing     (tx1: attempt 受理、累計増加と同時に commit)
     -> confirmed           (3 秒以内の approved を tx2 で commit)
     -> held or expired     (累計 3 回未満の declined)
     -> payment_failed      (累計 3 回目の declined)
     -> payment_unknown     (3 秒以内に結果を確定できない場合)
payment_unknown
  -> confirmed / held / expired / payment_failed
  -> unresolved_timeout    (15 分後も未解決)
```

- 決済試行の受理、Purchase Session の累計試行回数増加、`payment_processing` への遷移を 1 つ目の Aurora PostgreSQL transaction（tx1）で原子的に commit します。
- Fake Payment API の応答待ちでは Aurora PostgreSQL transaction と DB connection を保持しません。
- Fake Payment API が 3 秒以内に `approved` を返した場合は、2 つ目の transaction（tx2）で Purchase を確定します。
...
| 境界 | 目標値 | 責務 |
| --- | --- | --- |
| Fake Payment API outbound response deadline | 3 秒 | 同期確定を待つ上限。超過または transport / HTTP 5xx では `payment_unknown` へ進む |
| Purchase 確定 API application processing deadline | 5 秒 | 契約準拠応答または 5xx を返すまでのアプリケーション処理期限 |
| Amazon CloudFront origin response timeout | 30 秒 | origin から応答を待つエッジ側の上限 |
| Application Load Balancer idle timeout | 60 秒 | connection が idle のまま維持される上限 |
| Fastify target keep-alive timeout | 60 秒より長い値 | Application Load Balancer より先に target が keep-alive connection を閉じる競合を防ぐ |
...
Amazon CloudFront の 30 秒、Application Load Balancer の 60 秒、Fastify target の keep-alive は、実装時に Terraform とアプリケーション設定へ明示して暗黙の既定値依存をなくします。同じ経路を使う Next.js SSR への影響と、Amazon CloudFront / Application Load Balancer の 5xx が増えないことを staging full で検証します。
