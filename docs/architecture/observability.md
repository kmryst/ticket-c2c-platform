...
[ADR-0023](../adr/0023-split-b2c-purchase-journey-latency-sli.md) により、購入ジャーニーのレイテンシは単一の分布にせず、次の 2 つへ分ける。いずれも目標設計であり、メトリクスは未実装である。

| SLI | 境界 |
| --- | --- |
| 同期購入処理時間 | Session 交換、Ticket Hold 作成、Purchase 確定、結果確認の同期 API におけるプラットフォーム処理時間を扱う。Customer の操作時間と非同期の決済結果待ちは除外する |
| 決済結果解決時間 | Aurora PostgreSQL の `payment_processing` 遷移から、決済結果に基づく終端状態の確定までをサーバー側 wall-clock time で扱う |

各 API のレイテンシ SLI は診断用に維持し、`payment_unknown` の件数と滞留時間は Amazon CloudWatch Alarm で早期検知する。具体的なメトリクス名、業務 Outcome の分離、percentile、SLO 目標値、複数 API の集計・保存・emit 方式は未決定であり、現行 Purchase API の p95 800ms を自動的に流用しない。
