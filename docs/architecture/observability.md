...
各 API のレイテンシ SLI は診断用に維持し、`payment_unknown` の件数と滞留時間は Amazon CloudWatch Alarm で早期検知する。具体的なメトリクス名、対象 Outcome、percentile、SLO 目標値、複数 API の集計・保存・emit 方式は未決定であり、現行 Purchase API の p95 800ms を自動的に流用しない。
