...
**既存 22 本 + edge alarms 3 本（#252 実装済み） + 計画中の synthetic alarms（#256 で追加）**。
...
| severity | アラーム |
|---|---|
| **Critical** | `purchase-error-burn-rate-fast` / `alb-5xx` / `valkey-fail-open` / `aurora-freeable-memory-low` / `cloudfront-5xx-rate`（#252） / 計画中 `synthetic-check-failure`（#256、本数未確定） |
