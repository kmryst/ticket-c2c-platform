...
このドキュメントは ticket-c2c-platform の実装済み可観測性構成と運用方針の正本です。
設計判断の経緯は [ADR-0014](../adr/0014-xray-distributed-tracing-with-adot-sidecar.md) を参照してください。

| 文書 | 責務 |
| --- | --- |
| この文書 | 実装済みの分散トレーシング、メトリクス、アラーム、Dashboard、severity / escalation、runbook 導線 |
| [B2C 購入ジャーニーの可観測性](b2c-purchase-journey-observability.md) | 未実装の B2C 目標フローに対する SLI / SLO と計測境界 |
| [可観測性 実地検証ログ](observability-verification-log.md) | dev / staging での発火方法、実測値、確認結果、証跡 |
...
- 環境ごとに X-Ray group（`ticket-c2c-<env>`、filter: `service("ticket-c2c-<env>-api") OR service("ticket-c2c-<env>-worker")`）を Terraform（observability モジュール）で作成している。
...
- 付与済みのログ: Worker の処理ログ（`indexed event` / `updated inventory` 等）、購入 API の在庫不整合エラーログ、`rate_limit_exceeded` セキュリティイベントログ、EventBridge 発行失敗ログ、および EMF record（`emf.ts`。出力時点で span がアクティブなもの）。EventBridge 発行失敗ログは診断用であり、未送信イベントの再送または検索 projection の復旧は行わない。この既知ギャップは [Production Readiness M-12](./production-readiness.md) で追跡する。
...
- **EMF とは**: Amazon CloudWatch Embedded Metric Format の略。メトリクス名、値、単位、dimension と `_aws` メタデータを含む構造化 JSON ログを CloudWatch Logs へ送り、CloudWatch がログからカスタムメトリクスを自動抽出する形式である。
- 実装: `src/observability/emf.ts`。EMF 形式の JSON を stdout へ 1 行出すだけ（awslogs → CloudWatch Logs → 自動抽出）。アプリケーションから `PutMetricData` を呼ばないため追加 IAM は不要で、メトリクス送信失敗を業務処理から分離できる。
...
- **信頼性境界**: EMF は監視・傾向把握・アラーム用の best-effort データであり、購入状態や監査件数の正本ではない。stdout 出力前のプロセス停止による欠損やログ配送層での稀な重複を許容し、業務上の正確な状態は Aurora PostgreSQL を正本とする。
...
| ProjectionOutcome | Count | Worker | Service, Operation, Outcome | 検索 projection の処理結果分類（Operation=InventoryChanged/EventMetadata、Outcome=applied/stale/legacy_ignore/error）。version guard で適用したか・stale で no-op したか・失敗したかを区別する（Issue #377 / ADR-0031） |
...
- キュー全体の滞留は SQS 標準メトリクス `ApproximateAgeOfOldestMessage` を併用する。現行の専用アラームは DLQ の `ApproximateNumberOfMessagesVisible` であり、source queue age・Worker 停止の監視は [Production Readiness L-30](production-readiness.md) で追跡する。WorkerProcessingLagMs は「正常系での消費までの遅延」を見る用途。
...
- SLO 目標値・閾値アラームは [ADR-0017](../adr/0017-purchase-api-slo-burn-rate.md) / Issue #227 で実装済み（次節参照）

## 購入 API の SLO 目標値と閾値アラーム（ADR-0017 / Issue #227）

購入 API の SLI（前節）に対して SLO 目標値を定め、CloudWatch metric math による短期検知用と持続検知用の独立したアラームを実装している。成功率は error budget に対する error rate の倍率を使うため burn rate と呼べるが、レイテンシは p95 を SLO 目標値で割った閾値倍率であり、error budget の消費速度ではない。実装は `terraform/modules/observability`。

現行実装は、短時間窓と長時間窓の両方が同時に閾値を超えた場合だけ発報する multi-window multi-burn-rate alert ではない。fast と slow は AND 条件で結合されず、それぞれ独立して発報する。
...
| `<name>-purchase-error-burn-rate-slow` | 同上 | error burn ratio > 3 / 5分を6期間連続 | 成功率 SLO からの持続的な逸脱 |
...
| `<name>-purchase-latency-burn-rate-fast` | `PurchaseRequestLatencyMs`（Outcome=success、p95） | p95 / 800ms > 2.0 / 5分 | レイテンシ SLO からの急激な逸脱 |
| `<name>-purchase-latency-burn-rate-slow` | 同上 | p95 / 800ms > 1.2 / 5分を6期間連続 | レイテンシ SLO からの持続的な逸脱 |
...
- **fast / slow の評価方法**: fast は直近の 5 分 period だけを評価する。slow は 30 分を 1 つの集計 window にするのではなく、5 分 period の閾値超過を 6 回連続で要求する。両者は独立アラームであり、相互の AND 条件はない。
- **technical_failure 絶対数アラーム**: 購入 API は低頻度なため、burn-rate の低トラフィックガード（5件/5分）を割り込む時間帯が多い。見逃し防止のため、絶対数の静的閾値アラームを別途併設している。
- **レイテンシ閾値倍率の式**: `IF(success_count >= 5, p95_latency / 800, 0)`。この値はレイテンシ SLO 目標に対する超過倍率であり、error budget の消費速度ではない。
...
- **既存名の扱い**: AWS アラーム名、Terraform resource / variable、runbook、実地検証記録に含まれる `purchase-*-burn-rate-*` は既存リソースとの対応を維持するため変更しない。特に latency 系の `burn-rate` は既存の物理名であり、意味は上記の p95 閾値倍率を正本とする。

### dev / staging 実地検証（2026-07-09〜10）

2026-07-09〜10、dev で `error-burn-rate-fast` / `technical-failure-weak` / `technical-failure-normal` の 3 アラームを technical_failure（HTTP 500）の実発生により実発火させ、ALARM 遷移・SNS action・OK 復帰まで確認した。slow burn / latency 系 3 アラームは `set-alarm-state` による SNS 配線確認、staging は 6 アラームの作成と SNS 配線の確認（軽量パターン）とした。実発火手順・実測値・タイムスタンプの詳細は [observability-verification-log.md「購入 API SLO burn-rate 実地検証（2026-07-09〜10）」](observability-verification-log.md#購入-api-slo-burn-rate-実地検証2026-07-0910) を参照。
...
## B2C 購入ジャーニーの SLI / SLO（目標設計）

B2C 移行後の購入ジャーニーに対する SLI / SLO は目標設計段階であり未実装で、現行 Purchase API の SLI / SLO を置き換えた状態ではない。
ADR-0022〜ADR-0027 に基づく詳細（成功率 SLI、レイテンシ SLI の分割、同期 API のサーバー側計測境界、決済結果解決時間の計測境界と SLO）は [b2c-purchase-journey-observability.md](b2c-purchase-journey-observability.md) を参照。
現行 Purchase API が稼働している間は、ADR-0016 / ADR-0017 の成功率 99.5%・p95 800ms が現役 SLO であり、正本は上記「購入 API の SLI」「購入 API の SLO 目標値と閾値アラーム」節のまま変わらない。
B2C では計測を実装した後、Product 要件、staging full で観測したトラフィック特性、リクエスト数、エラー率、レイテンシ分布を基に SLO を確定する。その後、短時間窓と長時間窓を AND 条件で組み合わせる本格的な multi-window multi-burn-rate alert を設計する。詳細な決定順序は [B2C 購入ジャーニーの可観測性](b2c-purchase-journey-observability.md#b2c-の本格アラーム設計) を参照する。

## CloudWatch アラーム（Issue #218）

SRE の Four Golden Signals（Errors / Availability / Saturation / Latency 相当）を、既存パターン（sqs モジュールの DLQ アラーム）に倣い「リソースを所有する terraform モジュール内に配置し、`alarm_actions` を root module から注入」する形で実装している。この節で扱う東京リージョンのアラームは observability モジュールのアラート用 SNS トピック（`<name>-alerts` → email。L-5 / Issue #200）へ ALARM / OK 両遷移を通知する。us-east-1 の CloudFront / WAF / Synthetics アラームは、後述の `<name>-edge-alerts` へ通知する。
...
2026-07-09、`terraform-apply-dev` 実行後の dev 環境で実地検証を実施した。valkey-fail-open / worker-processing-lag / alb-5xx / alb-api-unhealthy-hosts / api-cpu-high / aurora-acu-near-max / aurora-cpu-high の 7 アラームは SG の一時 revoke や負荷生成により実発火させ、ALARM 遷移・SNS action の実行・OK 復帰まで確認した。api-memory-high / aurora-freeable-memory-low / aurora-connections-high の 3 アラームは `set-alarm-state` による SNS 配線確認とした。発火方法・実測値・タイムスタンプ・前提対応（新規 Aurora / 空 ECR への対処）の詳細は [observability-verification-log.md「CloudWatch アラーム実地検証（2026-07-09）」](observability-verification-log.md#cloudwatch-アラーム実地検証2026-07-09) を参照。

**運用上の注意**: 新規作成した CloudWatch アラームは、作成直後の初回評価までに数分〜十数分のラグが観測された（`no datapoints were received` の状態が続いた後に実データで評価される）。実発火試験ではこのラグを見込んで負荷を維持し続ける必要がある。
...
- **`aws_cloudfront_monitoring_subscription`（有料アドオン）**: `OriginLatency` 等の additional metrics の有効化に必要なため採用する。CloudFront は distribution ごとに最大 8 個の additional metrics を us-east-1 へ送り、CloudWatch はメトリクスごとの固定料金を課金する（[AWS 公式ドキュメント](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/viewing-cloudfront-metrics.html)）。リクエスト数に比例する料金ではないため、dev / staging を月途中で destroy する運用でも月内の実費は Cost Explorer で確認する（L-16 設計判断）。
...
- 3 アラームとも `treat_missing_data = notBreaching`（理由は上記「CloudWatch アラーム」節参照）。ALARM / OK 両遷移を us-east-1 側 SNS トピックへ通知する。
- severity プレフィックス（`[Critical]` / `[Warning]`）を `alarm_description` に付与済み（Issue #252 / #257）。
...
- canary run 料金は 1 run あたり約 $0.0012。5 分間隔で 24 時間稼働し続けた場合、run 料金だけで 288 run/日 ≈ $0.35/日 ≈ 月換算 $10 程度になる。これとは別に Lambda、CloudWatch Logs、S3、CloudWatch custom metrics、Alarm の料金が発生する（[CloudWatch 料金](https://aws.amazon.com/cloudwatch/pricing/)）。dev / staging は destroy 前提運用のため、実際の課金対象は環境の稼働時間に限られる。
...
`treat_missing_data = notBreaching`（理由は上記「CloudWatch アラーム」節参照）。ALARM / OK 両遷移を us-east-1 側 SNS トピック（`<name>-edge-alerts`）へ通知する。発報時の初動対応は [`docs/runbooks/alarm-synthetic-check-failure.md`](../runbooks/alarm-synthetic-check-failure.md)（Issue #272）を参照。severity は Critical（理由は下記「アラームの severity と escalation 方針」節参照）。

### destroy 運用での残存リソース対策（実地確認済み）
...
`terraform fmt -check -recursive terraform` と dev / staging / bootstrap 全 root の `terraform validate` が通ることを確認済み。加えて `terraform plan -target=module.synthetic_check`（dev / staging 両方、実バックエンドに対して実行）で、S3 バケット・IAM ロール・canary 本体・失敗アラームの計 9 リソースがエラーなく作成計画されることを確認した（canary スクリプトの zip 化（`archive_file`）も実際にファイルハッシュを計算し成功している）。bootstrap 側の apply ロール IAM ポリシー変更（`synthetics:*` 系アクション・`iam:PassRole` の追加）も `terraform plan -target=module.github_oidc` で意図どおりの追加差分のみ（既存権限の削除なし）であることを確認した。apply 後の実地検証（2026-07-12、Issue #276 での canary 成功 run・alarm action 配線確認）と destroy 後の残存リソース確認（2026-07-19）の経緯は [observability-verification-log.md「Synthetics canary 実地検証」](observability-verification-log.md#synthetics-canary-実地検証) を参照。
...
現行アラームの短期検知、持続検知、`-weak` / `-normal` という運用上の役割が 3 段階へ自然に写像できるため、3 段階を採用する。2 段階では「weak（初動不要の早期シグナル）」の受け皿がなくなる。
...
- `purchase-latency-burn-rate-{fast,slow}` は既存の物理名で、実体は p95 閾値倍率アラームである。「遅いが失敗していない」ため Warning とする（購入が失敗する error 系との差別化。ADR-0017 の役割分担どおり）。
...
### 通知チャネル: email 1 宛先、リージョン別 SNS 2 トピックを維持

通知先 email は 1 アドレスだが、CloudWatch alarm と SNS topic のリージョン制約により、通知経路は次の 2 トピックで構成する。

| リージョン | SNS topic | 対象 |
| --- | --- | --- |
| ap-northeast-1 | `<name>-alerts` | ALB / ECS / Aurora / SQS / EMF / 購入 API SLO |
| us-east-1 | `<name>-edge-alerts` | CloudFront / WAF / Synthetics |

両トピックは同じ `alert_email` を購読先に使う。環境を作り直した場合は、リージョンごとに届く 2 通の subscription confirmation が必要になる。このリージョン分割は severity routing ではなく AWS の制約によるものであり、Critical / Warning / Info は同じ宛先へ通知する。severity 別に通知先を分けない理由:
...
3. severity 別トピックを追加すると、既存のリージョン別 2 subscription に加えて apply ごとの Confirm 作業が増える（destroy 運用と相性が悪い）。
4. ADR-0017・L-16（`production-readiness.md`）の「規模に対して過剰な構成は採らない」前例と整合。

severity の区別は通知経路ではなく、全アラームへ付与済みの `alarm_description` severity プレフィックスで email 件名 / 本文から判別する。併発エスカレーション（前節）の判断も、通知経路の自動分岐ではなく「本人が受信メールを読んだ際に severity を再判定する」運用手順として位置づける（= Terraform 側の `alarm_actions` は分岐させない）。

**prod 化時の再検討条件**（いずれか成立で severity 別 routing を再検討）:
...
- **SNS の severity 別 routing（critical / non-critical）**: 上記条件成立時の第一選択。既存のリージョン分割を前提に、各リージョンのアラームをどの topic へ送るかを設計する。
...
Critical のうち併発エスカレーションの判定シグナルに使う `alb-5xx`、`purchase-error-burn-rate-fast`、`synthetic-check-failure` を受信した際は、CloudWatch コンソールで同時刻の Warning 系（`unhealthy-hosts`、Aurora 容量系）の ALARM 状態も併せて確認し、併発していれば根本原因の当たりをつける（前節「併発エスカレーション運用」参照）。その他の Critical は、対応する runbook に従って初動確認する。
...
既存アラームのリネームはしない（実地検証記録・ドキュメントとの整合を壊すため）。severity はこの節の対応表 + `alarm_description` のプレフィックスで表現する。

### 実装済み事項と将来の再検討

- 全アラームの `alarm_description` に `[Critical]` / `[Warning]` / `[Info]` を付与済み（Issue #272）。dev / staging の代表 severity と edge alarms は実環境でも確認済み。詳細は [可観測性 実地検証ログ](observability-verification-log.md#observability-一回通し実地検証2026-07-1213) を参照する。
- us-east-1 の edge 3 アラームと単一の `synthetic-check-failure` にも実装時から severity を付与済み（Issue #252 / #256）。
- SNS の severity 別 routing は、prod 化または上記の再検討条件成立時に設計する。東京リージョンの observability モジュールと us-east-1 の edge / synthetic alarm の両方が対象になる。
- 併発エスカレーション基準はドキュメント運用のままとする。Composite Alarm や EventBridge で自動化する場合は、ADR-0017 の不採用判断を覆すコスト対効果を別 ADR で記録する。
...
| アラーム群 | runbook |
| --- | --- |
| 購入 API SLO / 閾値アラーム / technical_failure | [`docs/runbooks/alarm-purchase-api-slo.md`](../runbooks/alarm-purchase-api-slo.md) |
...
| `OTEL_SERVICE_NAME` | `ticket-c2c-dev-api` | X-Ray サービスマップ上のノード名 |
