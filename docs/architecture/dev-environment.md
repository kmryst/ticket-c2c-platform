...
| S3 Gateway VPC Endpoint（ECR イメージレイヤー含む） | $0（Gateway 型は無料） |
| 合計 | **~$140/月** |

常駐コストの主因は OpenSearch / NAT / ALB。検証しない期間は destroy workflow で環境ごと削除するため、実際の月額は稼働時間に比例する。ecr.api / ecr.dkr / logs の Interface VPC Endpoint（月額約 $61）は、損益分岐点分析（1.355 GB/稼働時間）で dev/staging の実測トラフィックがその約 1/100 と判明したため撤去した（[ADR-0019](../adr/0019-remove-ecr-logs-interface-endpoints.md)、M-6 / Issue #313・#315）。撤去前の試算根拠・本番規模での再検討条件は [production-readiness.md](./production-readiness.md) の M-6 行を参照。
