...
|---|---:|---|---|
| `alb-http-only` | 初回構築で使用（fallback として維持） | ALB HTTP listener のみ。ACM 証明書なし、HTTPS listener なし、Route53 alias なし | `http://<alb_dns_name>` |
| `https-dns` | 現行の既定（Issue #94） | `ticket-api-staging.ticket-c2c.click`、ACM DNS 検証、ALB HTTPS listener、Route53 alias（HTTP は 301 リダイレクト） | `https://ticket-api-staging.ticket-c2c.click` |

初回 staging は `alb-http-only` で構築・検証した（[ADR-0008](../adr/0008-staging-ephemeral-prod-like-environment.md)、Issue #91）。実装順序 step 7 として `https-dns` を有効化し（Issue #94）、`terraform-apply-staging.yml` の `public_endpoint_mode` 入力（既定 `https-dns`）で切り替える。Terraform 変数の既定値は ADR-0008 のとおり `alb-http-only` のまま維持し、workflow 側が明示的に値を渡す。`https-dns` を指定した場合のみ ACM 証明書・DNS 検証レコード・Route53 alias・HTTPS リスナーが作成される。smoke test 等の base URL は Terraform output `api_base_url` で取得し、smoke test は https の場合に HTTP→HTTPS の 301 リダイレクトも検証する。
...
- 実行経路は 2 つ。いずれも ECS run-task（API タスク定義 + command override）で private subnet 内から適用する:
  ...
  - `deploy-app-*.yml` の `run_migrations` 入力: 新イメージのタスク定義 register 後・サービス更新前に migration を実行し、成功した場合のみデプロイへ進む（スキーマ変更を含むリリース用。migration 適用〜サービス更新完了までの間、旧タスクが新スキーマ上で動くため、migration は後方互換（expand-contract）で書く）。
...
terraform-apply-staging.yml
  ...
  -> db-migrate-staging.yml（初回構築時。スキーマ変更を含むリリースは deploy-app の run_migrations 入力で migration 成功後にサービス更新）
...
- base URL: Terraform output `api_base_url`（`https-dns` では `https://ticket-api-staging.ticket-c2c.click`、`alb-http-only` では `http://<alb_dns_name>`）
...
Aurora と Valkey の断時間の非対称性（84.2秒 vs 18.2秒）は、アプリ側の DB クライアントエラーハンドリングの差（`pg.Pool` はチェックアウト中 client のエラーを未捕捉のままプロセスを落とす一方、`ioredis` 側は同種の切断を例外として通常のリクエスト処理内で吸収する）に起因する。詳細と修正方針は Issue #108 を参照。
