...
- SLI の Outcome は、Aurora PostgreSQL の終端状態とジャーニー識別子を正本とする。
- 終端状態遷移の条件付き UPDATE と commit に成功した API または Worker だけが、commit 後に Outcome を Amazon CloudWatch Embedded Metric Format（EMF）で at-most-once 出力する。厳密な exactly-once のための recorded flag と transactional outbox は採用しない。
- commit 後かつ EMF 出力前のプロセス停止による稀な欠損と、ログ配送層で発生し得る稀な重複を許容する。受入検証や incident の事後検証で正確な件数が必要な場合は、Aurora PostgreSQL の終端状態件数と EMF メトリクスを突き合わせる。
...
- Aurora PostgreSQL の終端状態を Outcome の正本にすることで、開始時点と最大 15 分後の結果を同じ CloudWatch 集計期間へ無理に対応付けず、正確な事後集計も可能にする。
- 条件付き状態遷移に成功した主体だけが EMF を出力することで、API と Worker による二重 emit を抑制しつつ、計測専用の DB 書き込みを追加しない。
...
- EMF は best-effort のため、稀な欠損または重複が短期間・低トラフィック時の成功率に影響する可能性がある。業務状態と正確な件数の正本は Aurora PostgreSQL とし、乖離が無視できない場合は記録方式を再検討する。
...
- Aurora PostgreSQL の終端状態件数と EMF メトリクスに継続的な乖離が確認されたとき、または incident の事後検証で欠損・重複が SLO 判定に影響したとき。
