...
# 欠落のみで機能には影響しない（Issue #212）。ロググループを本プロジェクトの命名規則
# （プレフィックス ticket-c2c-*）で作れない AWS 側の固定パスのため、Resource は
# このロググループ ARN に限定しつつ、bootstrap 側の apply ロール権限（プレフィックス限定）とは
# 別に task role へ直接付与する。
...
data "aws_iam_policy_document" "cf_logs_delivery" {
  statement {
    ...
    resources = ["${aws_s3_bucket.cf_logs.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*"]
...
resource "aws_s3_bucket_policy" "cf_logs" {
  bucket = aws_s3_bucket.cf_logs.id
  policy = data.aws_iam_policy_document.cf_logs_delivery.json
}
...
resource "aws_cloudwatch_log_delivery" "cf_access" {
  ...
  delivery_destination_arn = aws_cloudwatch_log_delivery_destination.cf_logs_s3.arn
...
resource "aws_s3_bucket_policy" "waf_logs" {
  ...
  bucket = aws_s3_bucket.waf_logs.id
  policy = data.aws_iam_policy_document.waf_logs_delivery.json
}
...
# dev は module.cloudfront が count なしの必須リソースのため、edge widget は常に表示する。
module "dashboard" {
  ...
  ecs_frontend_service_name = module.frontend_service.service_name
  ...
  waf_web_acl_name           = "${var.name}-app-waf"
