...
ユーザーが明示的に `main` への直接 commit / push を許可した場合のみ、`main` へ直接 push してよいです。
その場合も、対象差分を確認し、明示的に stage してから commit / push します。

PR がマージされた後、次の Issue へ進む前に原則として `cleanup-merged-pr-branch.sh` を実行します。
...
実装や PoC によって設計・運用・構成が変わった場合は、まず `docs/` 配下の該当ドキュメントを更新します。

トレードオフを伴う設計判断は、後で ADR として残す候補にします。現時点では ADR ディレクトリは未作成のため、必要になった時点で `docs/adr/` と ADR 運用ルールを追加します。
